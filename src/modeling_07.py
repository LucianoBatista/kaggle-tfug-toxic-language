...
class Exp07:
    ...
    def train_tokenizer(self, df: pl.DataFrame):
        """
        Train tokenizer on X_train
        """
        train_texts = df.to_numpy().flatten().tolist()
        self.tokenizer = self.tokenizer.train_new_from_iterator(train_texts, 32000)

    def tokenize(self, df: pl.DataFrame) -> list[str]:
        """
        Tokenize text
        """
        train_texts = df.to_numpy().flatten().tolist()
    ...
    #
    def exp07_model(self, x_train, y_train):
        """
        Train and evaluate model
        """
...
if __name__ == "__main__":
    ...
    exp07.train_tokenizer(X_train)
    X_train_tokenized = exp07.tokenize(X_train)
    X_test_tokenized = exp07.tokenize(X_test)
    # X_train_tokenized = X_train.to_numpy().flatten().tolist()
    # X_test_tokenized = X_test.to_numpy().flatten().tolist()
    exp07.train_tfidf(X_train_tokenized)
    X_train = exp07.vectorize(X_train_tokenized)
    X_test = exp07.vectorize(X_test_tokenized)
    # exp06.exp06_w_cv()
    exp07.exp07_model(X_train, y_train)
