...
def call_gpt(examples: pl.DataFrame, new_example) -> str:
    examples = examples.sample(fraction=1, seed=2, shuffle=True).to_numpy()

    prompt = """Abaixo você encontra exemplos de comentários classificados entre tóxicos e não tóxicos.
    ...
    Agora classifique o seguinte comentário como tóxico ou não tóxico, responda apenas com a palavra "Tóxico" ou "Não Tóxico".
    Comentário: {}
    Classe:""".format(
        examples[0][0],
        ...
        examples[6][0],
        "Tóxico" if examples[6][1] == 1 else "Não Tóxico",
        examples[7][0],
        "Tóxico" if examples[7][1] == 1 else "Não Tóxico",
        examples[8][0],
        "Tóxico" if examples[8][1] == 1 else "Não Tóxico",
        examples[9][0],
        "Tóxico" if examples[9][1] == 1 else "Não Tóxico",
        new_example,
    )
    ...
    response = client.chat.completions.create(
        messages=messages, model="gpt-3.5-turbo", temperature=0.7, top_p=1
    )
...
if __name__ == "__main__":
    ...
    df.write_csv("data/train_gpt_check.csv")
